...
class Demo extends Component {
	...
	getResultsArea(lng, lat) {
		...
		fetch(url, { method: 'GET', headers: headers })
			...
			.then((json) => {
				...
				for (let i = 1; i <= 3; i++) {
					const priceM =
						json['2017Q' + i][this.getKeyNumber(json['2017Q' + i], 0)]['1'][
							'o_pu'
						];
					priceMun.push(priceM);
					const priceN =
						json['2017Q' + i][this.getKeyNumber(json['2017Q' + i], 1)]['1'][
							'o_pu'
						];
				...
				for (let i = 1; i <= 3; i++) {
					const priceM =
						json['2016Q' + i][this.getKeyNumber(json['2016Q' + i], 0)]['1'][
							'o_pu'
						];
					priceMun.push(priceM);
					const priceN =
						json['2016Q' + i][this.getKeyNumber(json['2016Q' + i], 1)]['1'][
							'o_pu'
						];
					priceNeighbourhood.push(priceN);
				}
				for (let i = 1; i <= 3; i++) {
					const priceM =
						json['2015Q' + i][this.getKeyNumber(json['2015Q' + i], 0)]['1'][
							'o_pu'
						];
					priceMun.push(priceM);
					const priceN =
						json['2015Q' + i][this.getKeyNumber(json['2015Q' + i], 1)]['1'][
							'o_pu'
						];
					priceNeighbourhood.push(priceN);
				}
	...
	getResultsIncome(lng, lat) {
		...
		fetch(url, { method: 'GET', headers: headers })
			...
			.then((json) => {
				const income =
					json['2017Q3'][this.getKeyNumber(json['2017Q3'], 0)][
						'renthog_06_13_M'
					]['0'];
				this.setState({
	...
	getResultsSalesTime(lng, lat) {
		...
		fetch(url, { method: 'GET', headers: headers })
			...
			.then((json) => {
				const districtC =
					json['2017Q3'][this.getKeyNumber(json['2017Q3'], 0)]['1']['s_t'];
	...
	getResultsServices(lng, lat) {
		...
		fetch(url, { method: 'GET', headers: headers })
			...
			.then((json) => {
				const schools =
					json['2017Q3'][this.getKeyNumber(json['2017Q3'], 0)]['PA_EDU_C'];
				const daycare =
					json['2017Q3'][this.getKeyNumber(json['2017Q3'], 0)]['PA_EDU_G'];
				const parkings =
					json['2017Q3'][this.getKeyNumber(json['2017Q3'], 0)]['PA_P'];
				const transport =
					json['2017Q3'][this.getKeyNumber(json['2017Q3'], 0)]['PA_TP'];
				this.setState({
					key_schools: schools,
					key_daycare: daycare,
			...
			<section className='demo' id='demoSection'>
				<div className='block__container'>
					...
					<p className='demo__text'>
						Introduce una dirección y prueba ejemplos de visualización de
						nuestras APIs.
					</p>
					<div className='demo__form'>
						...
						<Area
							city={this.state.priceMun}
							neighborhood={this.state.priceNeighbourhood}
						/>
						<div className='carto-graphics'>
							<Carto />
							<div className='income-sales'>
								<Income
									incomeNeighborhood={this.state.homeIncomeNeighbourhood}
								/>
								<Salestime
									salesTimeDistrict={this.state.timeDistrict}
									salesTimeNeighbourhood={this.state.timeNeighbourhood}
								/>
							</div>
							<Services
								servicesSchools={this.state.key_schools}
								servicesDaycare={this.state.key_daycare}
								servicesParkings={this.state.key_parking}
								servicesTransport={this.state.key_transport}
							/>
						</div>
					</div>
				</div>
